    ...
    def ask(self):
        ...
        with open(self.__filename, "r") as file: # go through database to see if question fuzzily matches with anything, if so, answer the question, else, learn the question
            for line in file:
                ...
                if match.ratio() > 0.9: # returns a ratio representing how much the current question matches a specific question in database
                    print(line.split(">>", 1)[1].strip()) # if 75% match, give the answer
                    self.__expectation = input("Is this what you expected (Y/N): ")
                    if self.__expectation.lower() == "y": return
                    break
